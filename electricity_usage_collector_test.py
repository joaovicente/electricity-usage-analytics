...
def test_electricity_usage_collector_first_collection():
    # First collection
    collector = ElectricityUsageCollector(username="username", password="password", storage_path=None, dry_run=False,
...
def test_electricity_usage_collector_second_collection():
    # second collection
    collector = ElectricityUsageCollector(username="username", password="password", storage_path=None,
                                          dry_run=False, runtime_mode=RuntimeMode.TEST)
...
def test_consecutive_day_collection():
    ...
    # first day
    collector = ElectricityUsageCollector(username="username", password="password", storage_path=storage_path,
                                          dry_run=False, runtime_mode=RuntimeMode.TEST)
...
def test_list_s3_objects():
    ...
    if os.environ.get('AWS_ACCESS_KEY_ID', None) is not None\
            and os.environ.get('AWS_SECRET_ACCESS_KEY', None) is not None:
